...
class All_tache extends MY_Controller {
	...
	public function index() {
		...
		$this->page = "templates/v3/All_tache.php";
		$this->layout();
	}
...
public function uptates_information_rapport() {
    ...
    $rapportConversions = $this->input->post('rapportConversions');
    $rapportConvCa = $this->input->post('rapportConvCa');
    $bilan = $this->input->post('bilan');
    $remarque = $this->input->post('remarque');

    // Appeler la méthode du modèle pour mettre à jour les données
    $updateResult = $this->Donne_modele->update_information_rapport(
        $idonnee,
        $rapport,
        $rapportConversions,
        $rapportConvCa,
...
public function info_am() {
    ...
    $this->form_validation->set_rules('idupsell', 'ID Upsell', 'required|is_numeric');
    $this->form_validation->set_rules('information_upsell', 'Information AM', 'required');
    ...
    $idupsell = $this->input->post('idupsell');
    $information_upsell = $this->input->post('information_upsell');

    // Appeler le modèle pour mettre à jour les données
    $updateResult = $this->Donne_modele->update_information_upsells($idupsell, $information_upsell);
...
public function info_tech() {
    ...
    $this->form_validation->set_rules('idupsell', 'ID Upsell', 'required|numeric');
    $this->form_validation->set_rules('information_am', 'Information AM', 'trim');

    // Vérification si la validation échoue
    if ($this->form_validation->run() === FALSE) {
        echo json_encode([
            'status' => 'error',
            'message' => strip_tags(validation_errors())  // Renvoie les erreurs de validation
        ]);
        return;
    }

    // Récupère les données du formulaire
    $idupsell = $this->input->post('idupsell', TRUE);
    $information_am = $this->input->post('information_am', TRUE);

    // Log pour vérifier si l'ID est bien récupéré
    log_message('error', 'ID Upsell reçu: ' . $idupsell);  // Vérifie les logs de CodeIgniter

    if (empty($idupsell)) {
        echo json_encode([
            'status' => 'error',
            'message' => "Le champ ID Upsell est requis."
        ]);
        return;
    }

    // Mets à jour les informations dans la base de données
    $updateResult = $this->Donne_modele->update_information_ams($idupsell, $information_am);

    echo json_encode([
        'status' => $updateResult ? 'success' : 'error',
        'message' => $updateResult ? 'Mise à jour réussie' : 'Erreur lors de la mise à jour'
    ]);
}
...
public function update_etat() {
    ...
    $this->form_validation->set_rules('idupsell', 'ID Upsell', 'required|numeric');
    $this->form_validation->set_rules('etat', 'État', 'required|in_list[0,1,2]');
    ...
    $id = $this->input->post('idupsell', TRUE);
    $etat = $this->input->post('etat', TRUE);

    $success = $this->Donne_modele->update_etat_upsell($id, $etat);
...
public function update_etat_am() {
    ...
    $this->form_validation->set_rules('idupsell', 'ID Upsell', 'required|numeric');
    $this->form_validation->set_rules('etat_am', 'État AM', 'required|in_list[0,1,2]');
    ...
    $id = $this->input->post('idupsell', TRUE);
    $etat_am = $this->input->post('etat_am', TRUE);

    // Debugging dans les logs pour vérifier les valeurs
    log_message('error', "ID Upsell: " . $id . " - État AM: " . $etat_am);

    // Effectuer la mise à jour dans la base de données
    $success = $this->Donne_modele->update_etat_am($id, $etat_am);
