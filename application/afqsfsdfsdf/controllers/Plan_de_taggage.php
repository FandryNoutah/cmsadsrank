...
class Plan_de_taggage extends MY_Controller {
	...
	public function index() {
		...
		$this->page = "templates/v3/Plan_de_taggage.php";
		$this->layout();
	}
		...
		if ($this->ion_auth->logged_in()) {
			...
			$this->page = "templates/v3/plan_de_taggage2";
			$this->layout();
		}
	}
...
public function edit_task() {
	...
	$this->Task_model->update_task($taskId, $title,$am, $date_demande, $date_due, $description, $assignedTo, $status);

	// Rediriger vers une page avec un message de succès
	$this->session->set_flashdata('message', 'Tâche mise à jour avec succès');
	redirect('Plan_de_taggage/plandetaggage/' . $idclients);
}
public function add_task() {
	...
	$date_due = $this->input->post('date_due');
	$idclients = $this->input->post('idclients');
	$type_tache = $this->input->post('type_task');
	$type_tache = intval($type_tache);
...
public function Ajoutplantaggage() {
	...
	$this->visuels_model->insert_plan_de_taggage($data_extensions);
		...
		redirect('Plan_de_taggage/plandetaggage/' . $idclients, 'refresh');
	$this->layout();
}
...
public function ajoutplandetaggage($id) {
	$this->data["clients"] = $this->visuels_model->getClientById($id); 
	$this->page = "templates/v3/ajout_plan_de_taggage2";
	$this->layout();
...
public function deleteplandetagggage($id) {
	...
	public function insertgroupeannonce($id) {
		...
		public function save_groupe() {
			...
			redirect('Plan_de_taggage/validation_client/' . $idgroupe_annonce);
			...
			redirect('Plan_de_taggage/success');  // Exemple de redirection après mise à jour
		}
	...
	public function save_brouillon($idonnee) {
		...
		redirect('Plan_de_taggage', 'refresh');
		$this->layout();
	}
	...
	public function save_annonce($idcampagne) {
		...
		$this->session->set_flashdata('message-succes', "Campagne sauvegarder");
		redirect('Plan_de_taggage/admin_brief/' . $idclients);
		$this->layout();
	}
...
public function Ajout_Campagne() {
    if ($this->input->post('type_de_campagne') == 1) {
        ...
        redirect('Plan_de_taggage/campagne/'. $idclients, 'refresh');
        $this->layout();
	...
	public function updateinformationClient() {
		...
		redirect('Plan_de_taggage/Admin_brief/'. $idonnee, 'refresh');
	}
