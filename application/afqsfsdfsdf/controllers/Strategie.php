...
class Strategie extends MY_Controller
{
	...
	public function index() {
		...
		$this->data['All_task_non_complete'] = $this->Task_model->get_All_task_non_complete();
		$this->data['tache_team'] = $this->Task_model->get_task_team();
		$this->data['tache_team_non_complete'] = $this->Task_model->get_team_task_non_complete();
		$this->data['temporaire'] = $this->Task_model->get_task_temporaire();
		$this->data['temporaire_non_complete'] = $this->Task_model->get_temporaire_task_non_complete();
		$this->data['gtm'] = $this->Task_model->get_task_gtm();
		$this->data['gtm_non_complete'] = $this->Task_model->get_gtm_task_non_complete();
		...
		$this->page = "templates/v3/admin-strategie.php";

		$this->layout();
	}

	public function get_task_by_idtask($idtask) {
		...
		$task = $this->Task_model->get_task_by_id($idtask);
		...
		if ($task) {
			...
			// Répondre avec les données de la tâche sous forme JSON
			echo json_encode($task_data);
		} else {
			// Si aucune tâche n'est trouvée, renvoyer une erreur
			echo json_encode(['error' => 'Tâche non trouvée']);
		}
	}
	public function update_task($idtask) {

		// Récupérer les données envoyées par le formulaire
		$task_data = $this->input->post();

		// var_dump($task_data);
		// die();
		// Validation des données

		$this->form_validation->set_rules('date_demande', 'Date de la demande', 'required');
		$this->form_validation->set_rules('description', 'Description', 'required');
		$this->form_validation->set_rules('assigned_to', 'Utilisateur assigné', 'required');
		$this->form_validation->set_rules('AM', 'Account Manager', 'required');
		$this->form_validation->set_rules('title', 'Leviers Marketing', 'required');
		$this->form_validation->set_rules('Statuts_technique', 'Status Technique', 'required');

		// Si la validation échoue
		if ($this->form_validation->run() == FALSE) {
			// Recharger la page de modification de la tâche avec les erreurs
			$this->edit_task($idtask);
		} else {
			// Si les données sont valides, mettre à jour la tâche
			$this->Task_model->update_task($idtask, $task_data);

			// Rediriger vers la page de stratégie ou une autre page après la mise à jour
			redirect('Strategie');
		}
	}
	...
	public function edit_task($idtask) {
		...
		// Récupérer les utilisateurs (AM et TM)
		$users = $this->Task_model->get_all_users();

		// Passer les données à la vue
		$this->data['task'] = $task;
		$this->data['users'] = $users;
		$this->page = "templates/v3/edit-task.php"; // Le chemin de votre vue
		$this->layout(); // Appeler la méthode layout() pour charger la vue
	}
	public function insert_tache() {
		...
		redirect('Strategie', 'refresh');
	}
	public function insert_tache_temporaire() {
		// Récupérer les données envoyées par le formulaire
		$type_tache = $this->input->post('type_tache');
		$date_demande = $this->input->post('date_demande');
		$date_due = $this->input->post('date_due');
	...
	public function insert_tache_gtm() {
		// Récupérer les données envoyées par le formulaire
		$type_tache = $this->input->post('type_tache');
		$date_demande = $this->input->post('date_demande');
		$date_due = $this->input->post('date_due');
	...
	public function updates_task($task_id) {
		$data = array(
			'status' => $this->input->post('status')
		);
	...
	public function ajout_type_plan_taggage() {
		...
		if ($choix == "lead") {
			$conversions = [
				['idclients' => $idclients, 'conversion' => 'Lead - Formulaire Page contact', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead - Demande de devis', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead - Téléphone', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead - Email', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Contact - Chat', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Bouton |', 'actions' => 'Secondaire', 'types' => 'Click', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Bouton |', 'actions' => 'Secondaire', 'types' => 'Click', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Bouton | Télécharger notre catalogue', 'actions' => 'Secondaire', 'types' => 'Click', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => '']
			];
		}
		if ($choix == "ecommerce") {
			$conversions = [
				['idclients' => $idclients, 'conversion' => 'Lead | Achat', 'actions' => 'Principale', 'types' => 'Purchase', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Formulaire Page contact', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Demande de devis', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Téléphone', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Email', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Contact - Chat', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Checkout', 'actions' => 'Secondaire', 'types' => 'begin_checkout', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Ajout au panier', 'actions' => 'Secondaire', 'types' => 'Add to cart', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue | Vue d\'un article', 'actions' => 'Secondaire', 'types' => 'View item', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Page vue |', 'actions' => 'Secondaire', 'types' => 'Page_view', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				...
				['idclients' => $idclients, 'conversion' => 'Bouton |', 'actions' => 'Secondaire', 'types' => 'Click', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Bouton |', 'actions' => 'Secondaire', 'types' => 'Click', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Newsletter', 'actions' => 'Secondaire', 'types' => 'inscription', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Création de compte Client', 'actions' => 'Secondaire', 'types' => 'inscription', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => '']
			];
		}
		if ($choix == "reservation") {
			$conversions = [
				['idclients' => $idclients, 'conversion' => 'Lead | Réservation', 'actions' => 'Principale', 'types' => 'Purchase', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Formulaire Page contact', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Demande de devis', 'actions' => 'Principale', 'types' => 'Formulaire', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Téléphone', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
				['idclients' => $idclients, 'conversion' => 'Lead | Email', 'actions' => 'Principale', 'types' => 'Contact', 'remarque' => '', 'etat' => '', 'conditions' => '', 'conversion_id' => '', 'conversion_label' => '', 'extensions_appel' => ''],
	...
	public function plandetaggage($id) {
		if ($this->ion_auth->logged_in()) {
			...
			$t = $this->data["tasks"] = $this->Task_model->get_all_tasks();
			...
			$this->page = "templates/v3/plan_de_taggage";
			$this->layout();
		}
	}
	...
	public function ajoutextension($id) {
		...
		$idclients = $this->visuels_model->getglientbycampagne($id);
	...
	public function updateDonneeClients() {
		...
		$this->visuels_model->updatescampagne($zones, $date_campagne, $appareil, $budget, $information_campagne, $idcampagne);
		...
		redirect('Googleads/campagne/' . $id);
	}
	...
	public function campagne($idclients) {
		...
		$donnees_valider = $this->Donne_modele->getcclientvalidation();
		$groupes_valider = $this->Donne_modele->getcampagnegroupevalidation();
	...
	public function Ajoutgroupepmax() {
		...
		$logos = $this->input->post('logos'); // On récupère le logo existant
		$favicon = $this->input->post('favicon'); // On récupère le favicon existant
		...
		// Vérification et téléchargement du favicon
		if ($_FILES['favicon']['name'] != '') {
			$this->upload->initialize($this->set_upload_options("", $_FILES["favicon"]["name"]));
			if ($this->upload->do_upload('favicon')) {
				$favicon = $this->path . $this->upload->file_name; // Nouveau favicon téléchargé
			}
		}
		...
		// Ajouter le favicon si nécessaire
		if ($favicon != "") {
