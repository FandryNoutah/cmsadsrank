...
class Doublon_personnelle extends MY_Controller {
	...
	public function index() {
		...
		$this->data['liste_Personnelle'] = $this->concurrent->get_Liste_Personnelle_doublon();	
		$this->page = "templates/v3/admin-doublon";
		$this->layout();
	}
	function fetch()
 {
 ...
 public function desabonner_Personnelle($id_personnelle){
		$this->concurrent->desabonner($id_personnelle);
		$this->session->set_flashdata('message-succes', "Personnelle desabonner ");
		redirect('Personnelle', 'refresh');
		$this->layout();
		
	}	
	public function update_personnelle($id_personnelle){
	
	$ko = $this->concurrent->get_Personnelle_by_id($id_personnelle);	
	$this->data["personnelle"] = $ko;
	$this->page = "templates/v3/admin_personnelle_edit";
	$this->layout();
		...
		$Ramassage_remisage=$this->input->post('Ramassage_remisage');
		$Adresse_exacte=$this->input->post('Adresse_exacte');
		$Contact=$this->input->post('Contact');
		$Lieu_de_travail=$this->input->post('Lieu_de_travail');
		$OBS=$this->input->post('OBS');
		//var_dump($Nom);
		//Die();
		$this->concurrent->update_personnelle($Nom,$Prenom,$Matricule,$Societe,$Direction,$Ramassage_remisage,$Contact,$Lieu_de_travail,$OBS,$Adresse_exacte,$id_personnelle);
	...
	public function delete_Personnelle($id_personnelle){
	        $this->concurrent->deletePersonnelle($id_personnelle);
 ...
 public function tranfrerer_Personnelle($id_personnelle){
	// var_dump("OK");
	// die();
	$liste_perso = $this->concurrent->get_personnelle_by_id($id_personnelle); 
	foreach($liste_perso as $l){
		$ligne = $l['Ligne'];
	}
	
	$this->data['detail_Personnelle'] = $this->concurrent->get_personnelle_by_id($id_personnelle); 
	$this->data['liste_car'] = $this->visuels_model->get_all_car();	
	$this->page = "templates/v3/transfer_personnelle";
	$this->layout();
		if($this->input->post('transferer'))
		{
			$id_personnelle=$this->input->post('id_personnelle');
			$Nom_Car=$this->input->post('Nom_Car');
			$listP = $this->concurrent->count_Personnelle($Nom_Car);
			$nombre_personnnelle_dans_le_car = count($listP);
			
			$Liste_Car = $this->concurrent->get_ligne_car_by_id($Nom_Car);
			foreach($Liste_Car as $l){
				$nbr_place = $l['Nombre_place'];
			}
	// $this->data['liste_car'] = $this->visuels_model->get_all_car();	
	
	
			$Nbr_place_car = substr($nbr_place,0,2);
			$Nbr_place_car_result = intval($Nbr_place_car);
			
			
			
			if($nombre_personnnelle_dans_le_car < $Nbr_place_car_result){
			
			
			$this->concurrent->transfer($Nom_Car,$id_personnelle);
			$this->session->set_flashdata('message-succes', "Tranfert effectuer");
			redirect('Personnelle', 'refresh');
			$this->layout();
			}else{
				...
				// var_dump($nombre_personnnelle_dans_le_car);
				// var_dump($Nbr_place_car_result);
	...
	public function insert_liste_personnelle()
	{
		$Nom = $this->input->post('Nom');
		$Prenom = $this->input->post('Prenom');
		$Matricule = $this->input->post('Matricule');
        ...
        $this->concurrent->insertpersonnelle($Nom,$Prenom,$Matricule,$Societe,$Direction,$Ligne,$Ramassage_remisage,$Adresse_exacte,$Contact,$Lieu_de_travail,$OBS);
		$this->session->set_flashdata('message-succes', "Donnée ajouté avec succès");
		redirect('Personnelle', 'refresh');
		$this->layout();
	}
 function insert_liste_personnelle_xls()
 {
  if(isset($_FILES["file"]["name"]))
  {
   ...
   foreach($object->getWorksheetIterator() as $worksheet)
   {
    ...
    for($row=2; $row<=$highestRow; $row++)
    {
	 ...
	 $Societe = $worksheet->getCellByColumnAndRow(1, $row)->getValue();
	 $Nom = $worksheet->getCellByColumnAndRow(2, $row)->getValue();
	 $Prenom = $worksheet->getCellByColumnAndRow(3, $row)->getValue();
	 $Direction = $worksheet->getCellByColumnAndRow(4, $row)->getValue();
	 $Ligne = $worksheet->getCellByColumnAndRow(5, $row)->getValue();
	 $Ramassage_remisage = $worksheet->getCellByColumnAndRow(6, $row)->getValue();
	 $Adresse_exacte = $worksheet->getCellByColumnAndRow(7, $row)->getValue();
	 $Contact = $worksheet->getCellByColumnAndRow(8, $row)->getValue();
	 $Lieu_de_travail = $worksheet->getCellByColumnAndRow(9, $row)->getValue();
     $OBS = $worksheet->getCellByColumnAndRow(10, $row)->getValue();
     $data[] = array(
      'Matricule'  => $Matricule,
	  'Societe'  => $Societe,
	  'Nom'  => $Nom,
	  ...
	  'Direction'  => $Direction,
	  'Ligne'  => $Ligne,
	  'Ramassage_remisage'  => $Ramassage_remisage,
   ...
   $this->concurrent->Insert_DATA($data);
   $this->session->set_flashdata('message-succes', "Donnée ajouté avec succès");
   redirect('Personnelle', 'refresh');
	$this->layout();
  } 
	...
	public function delete_concurrent($id){
		...
		$ko = $this->concurrent->get_car_by_id($id_Car); 
		//datadump($ko);
		//die();
