...
<div id="tab1" class="tab-content active-content">
    <form method="POST" action="<?= site_url('Googleads/vie/'.$id); ?>">
    ...
    <?php if (!empty($images)) : ?>
        <h2>Images récupérées :</h2></br>
        Veuillez sélectionner 8 images
        <form method="POST" action="<?= site_url('Googleads/save_images'); ?>" id="save-form">
            <button type="submit" id="save-button" style="background-color: #37BC9B" disabled>Enregistrer images</button>
...
<script>
    ...
    // Met à jour l'état du bouton "Enregistrer image" selon le nombre d'images sélectionnées
    function updateSaveButtonState() {
        const selectedCheckboxes = document.querySelectorAll('.image-checkbox:checked');
        if (selectedCheckboxes.length === maxSelection) {
            saveButton.disabled = false; // Activer le bouton si 8 images sont sélectionnées
        } else {
            saveButton.disabled = true; // Désactiver le bouton sinon
        }
    }

    // Ajoute un écouteur pour chaque case à cocher
    checkboxes.forEach(checkbox => {
        checkbox.addEventListener('change', function() {
            // Vérifie si le nombre d'images sélectionnées dépasse 8
            const selectedCheckboxes = document.querySelectorAll('.image-checkbox:checked');
            if (selectedCheckboxes.length > maxSelection) {
                this.checked = false; // Désélectionner cette case
                alert('Vous ne pouvez sélectionner que 8 images.');
            }
            // Met à jour l'état du bouton en fonction du nombre d'images sélectionnées
    ...
    saveForm.addEventListener('submit', function(event) {
        ...
        if (selectedCheckboxes.length !== maxSelection) {
            ...
            alert('Vous devez sélectionner exactement 8 images avant de soumettre.');
        }
    });
