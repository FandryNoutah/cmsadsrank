		...
		<div class="col" style="height: calc(100vh - 101px); overflow-y:auto;">
			<div class="container-fluid pb-5 pt-3">
				...
				<div class="tab-content" id="taskTabContent">
						...
						<div class="card-body py-5 px-4">
							<div class="row align-items-center">
								...
								<div class="col-3 text-center">
									...
									<?php else: ?>
										<h6> Demande de procédure effectuer</h6>
										<label class="toggle" aria-label="Activer procédure">
											<input type="checkbox" class="activer-procedure"
												data-idclient="<?php echo $d['idclients']; ?>"
												data-am="<?php echo $d['initiative']; ?>"
					...
					<div class="dropdown position-absolute" style="right: 15px; margin-top: -55px;"> <?php if ($current_user->tech == 3): ?> <?php if ($d['statut_brief'] == 1): ?> <a type="button" class="badge alert-success rounded-pill px-4 py-3 mb-3 " style="font-size: 14px; font-weight: 500;" id="clientStatusDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-circle mr-1" style="font-size: 14px;"></i> Brief complété </a> <?php endif; ?> <?php if ($d['statut_brief'] == 0): ?> <div class="dropdown mb-3"> <a class="badge alert-warning rounded-pill px-4 py-3 dropdown-toggle" href="#" id="clientStatusDropdown_<?= $d['idonnee'] ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="font-size: 14px; font-weight: 500;"> <i class="fa fa-circle mr-1" style="font-size: 14px;"></i> Brouillon </a> <div class="dropdown-menu" aria-labelledby="clientStatusDropdown_<?= $d['idonnee'] ?>"> <a href="#" class="dropdown-item text-primary sendToStructure" data-id="<?= $d['idonnee'] ?>"> Envoyer à la structure </a> </div> </div> <!-- Modal de confirmation --> <div class="modal fade" id="confirmSendModal_<?= $d['idonnee'] ?>" tabindex="-1" role="dialog"> <div class="modal-dialog" role="document"> <div class="modal-content"> <div class="modal-header"> <h5 class="modal-title">Confirmation</h5> <button type="button" class="close" data-dismiss="modal">&times;</button> </div> <div class="modal-body"> Voulez-vous envoyer à la technique ? </div> <div class="modal-footer"> <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button> <form action="<?= site_url('Client/send_to_technique/' . $d['idonnee']) ?>" method="post" style="display:inline;"> <button type="submit" class="btn btn-primary">Oui, envoyer</button> </form> </div> </div> </div> </div> <?php endif; ?> <?php endif; ?> <?php if ($current_user->tech == 1): ?> <?php if ($d['statut_envoye'] == 1): ?> <a type="button" class="badge alert-success rounded-pill px-4 py-3 mb-3 dropdown-toggle" style="font-size: 14px; font-weight: 500;" id="clientStatusDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false"> <i class="fa fa-circle mr-1" style="font-size: 14px;"></i> Annonce complété </a> <?php endif; ?> <?php if ($d['statut_envoye'] == 0): ?> <div class="dropdown mb-3"> <!-- Badge Annonce brouillon --> <a class="badge alert-warning rounded-pill px-4 py-3 dropdown-toggle" href="#" id="annonceStatusDropdown_<?= $d['idonnee'] ?>" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" style="font-size: 14px; font-weight: 500;"> <i class="fa fa-circle mr-1" style="font-size: 14px;"></i> Annonce brouillon </a> <div class="dropdown-menu" aria-labelledby="annonceStatusDropdown_<?= $d['idonnee'] ?>"> <a href="#" class="dropdown-item text-primary sendAnnonce" data-id="<?= $d['idonnee'] ?>"> Envoyer l’annonce </a> </div> </div> <!-- Modal de confirmation --> <div class="modal fade" id="confirmSendAnnonceModal_<?= $d['idonnee'] ?>" tabindex="-1" role="dialog"> <div class="modal-dialog" role="document"> <div class="modal-content"> <div class="modal-header"> <h5 class="modal-title">Confirmation</h5> <button type="button" class="close" data-dismiss="modal">&times;</button> </div> <div class="modal-body"> Voulez-vous vraiment envoyer cette annonce ? </div> <div class="modal-footer"> <button type="button" class="btn btn-secondary" data-dismiss="modal">Annuler</button> <form action="<?= site_url('Client/send_annonce/' . $d['idonnee']) ?>" method="post" style="display:inline;"> <input type="hidden" value="<?= $d['account_manager'] ?>" name="am"> <button type="submit" class="btn btn-primary">Oui, envoyer</button> </form> </div> </div> </div> </div> <?php endif; ?> <?php endif; ?> </div>
