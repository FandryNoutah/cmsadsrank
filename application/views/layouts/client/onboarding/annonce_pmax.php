...
<head>
  ...
  <title>Performance Max – Vue CI3 (base Search)</title>
...
$(document).ready(function() {
  ...
  /* === Aperçu mobile en temps réel (PMax) === */
  function hostFrom(url) {
    try { return new URL(url).host.replace(/^www\./,''); } catch { return 'exemple.com'; }
  }
  // PMax : pas de chemins => URL d'affichage = host uniquement
  function displayUrl() {
    return hostFrom($('#url_campagne').val().trim());
  }
  ...
  function updatePhonePreview() {
    ...
    const address = $('input[name="adresse"]').val ? $('input[name="adresse"]').val().trim() : '';
    if (address || phone) {
      let meta = '';
  ...
  // Saisie -> preview en temps réel (chemins retirés)
  $(document).on('input',
    'input[name="titres[]"], input[name="descriptions[]"], input[name="titre_annexe[]"], input[name="desc1_annexe[]"], input[name="desc2_annexe[]"], input[name="url_annexe[]"], input[name="accroche_annexe[]"], input[name="site_annexe[]"], input[name="numero"], input[name="adresse"], input[name="Description_brève"], #url_campagne',
