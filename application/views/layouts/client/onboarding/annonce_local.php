...
<head>
  ...
  <title>Campagne Local – Vue CI3 (base PMax)</title>
    ...
    <div class="col-12 col-lg-6 order-lg-2">
      <form action="<?= base_url('Client/Ajoutgroupes/' . $groupe[0]['idclients']) ?>" method="POST" enctype="multipart/form-data">
        <div class="container-fluid pt-4">
          ...
          <!-- Local : fiche / email / adresse -->
          <div class="g-block" id="sec-local">
            <div class="g-head" data-toggle="#body-local">
              <div>
                <div class="g-title">
                ...
                <div class="g-sub">Fiche établissement, email et adresse</div>
              </div>
            </div>
            <div class="g-body" id="body-local">
              <div class="g-field">
                <label class="g-label">Fiche Établissement envoyée au client</label>
                <select name="fiche_etablissement" class="form-control">
                  <option value="Oui">Oui</option>
                  <option value="Non">Non</option>
                </select>
              </div>
              <div class="g-field">
                <label class="g-label">Email du client</label>
                <input type="text" class="form-control g-input" name="email_clients" id="email_clients" data-max="80" maxlength="80" placeholder="ex: contact@exemple.com">
                <span class="g-counter">0/80</span>
              </div>
              <!-- visible 'adresse' + hidden 'adresse_campagne' (compat base) -->
              <div class="g-field">
                <label class="g-label">Adresse du client</label>
                <input type="text" class="form-control g-input" name="adresse" id="adresse" data-max="120" maxlength="120" placeholder="ex: 10 Rue de la Paix, 75002 Paris">
                <span class="g-counter">0/120</span>
                <input type="hidden" name="adresse_campagne" id="adresse_campagne">
              </div>
            </div>
          </div>
          ...
          <!-- Description brève -->
          <div class="g-block" id="sec-brief">
            <div class="g-head" data-toggle="#body-brief">
              <div>
                <div class="g-title"><svg width="16" height="16" class="g-arrow" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"></polyline></svg> Description brève</div>
                <div class="g-sub">Texte court utilisé sur certains placements</div>
              </div>
            </div>
            <div class="g-body" id="body-brief">
              <div class="g-field">
                <label class="g-label">Description brève</label>
                <input type="text" class="form-control g-input" name="Description_brève" data-max="60" maxlength="60">
          ...
          <div class="g-block" id="sec-images">
            <div class="g-head" data-toggle="#body-images">
              <div>
                ...
                <div class="g-sub">Ajoutez images/visuels pour Local</div>
              </div>
            </div>
          ...
          <div class="g-block" id="sec-sitelinks">
            <div class="g-head" data-toggle="#body-sitelinks">
              <div>
                ...
                <div class="g-sub">Jusqu'à 4 sitelinks (texte 25, 2 descriptions 90, URL)</div>
              </div>
            </div>
          ...
          <div class="g-block" id="sec-snippet">
            <div class="g-head" data-toggle="#body-snippet">
              <div>
                ...
                <div class="g-sub">Ajoutez des extraits (25 caractères max)</div>
              </div>
            </div>
          ...
          <div id="preview-section" class="preview-card mt-4 d-none">
            <h5 class="mb-3" id="preview-anchor">Aperçu de la campagne</h5>
            <table class="table table-bordered">
              <tr><th>Campagne</th><td><?= $groupe[0]['nom_campagne'] ?></td></tr>
              <tr><th>Groupe d'annonces</th><td><?= $groupe[0]['nom_groupe'] ?></td></tr>
              ...
              <tr><th>Description brève</th><td id="preview-brief">Aucune description brève</td></tr>
              <tr><th>Email</th><td id="preview-email">Aucun email</td></tr>
              <tr><th>Adresse</th><td id="preview-adresse">Aucune adresse</td></tr>
              <tr><th>Téléphone</th><td id="preview-numero">Aucun numéro</td></tr>
...
$(document).on('click', '.remove-annexe', function (e) {
  ...
  function updateTablePreview() {
    const titres = $('input[name="titres[]"]').map((_,el)=>el.value.trim()).get().filter(Boolean);
    $('#preview-titres').html(titres.length ? titres.join('<br>') : 'Aucun titre');

    const titresLongs = $('input[name="titres_longs[]"]').map((_,el)=>el.value.trim()).get().filter(Boolean);
    $('#preview-titres-longs').html(titresLongs.length ? titresLongs.join('<br>') : 'Aucun titre long');

    const descs = $('input[name="descriptions[]"]').map((_,el)=>el.value.trim()).get().filter(Boolean);
    $('#preview-descriptions').html(descs.length ? descs.join('<br>') : 'Aucune description');

    $('#preview-url').text($('#url_campagne').val().trim() || 'Aucune URL');

    const brief = $('input[name="Description_brève"]').val()?.trim() || '';
    $('#preview-brief').text(brief || 'Aucune description brève');

    const imgs = $('#propositionImagesContainer img').map((_,img)=>img.src).get();
    $('#preview-image').html(imgs.length ? imgs.map(src=>`<img src="${src}" width="120" style="margin:2px;object-fit:cover;">`).join('') : 'Aucune image');

    // Email + Numéro + Adresse
    $('#preview-email').text($('#email_clients').val()?.trim() || 'Aucun email');
    const num = $('input[name="numero"]').val()?.trim() || '';
    const adr = ($('#adresse').val()?.trim() || $('#adresse_campagne').val()?.trim() || '');
  ...
  /* === Aperçu mobile en temps réel (Local) === */
  function hostFrom(url) {
    try { return new URL(url).host.replace(/^www\./,''); } catch { return 'exemple.com'; }
  }
  // Local : pas de chemins => URL d'affichage = host uniquement
  function displayUrl() { return hostFrom($('#url_campagne').val().trim()); }
  ...
  function updatePhonePreview() {
    ...
    const brief = $('input[name="Description_brève"]').val()?.trim() || '';
    const desc = (brief ? [brief, ...ds] : ds).slice(0,2).filter(Boolean).join(' — ')
                 || "Votre description s'affiche ici. Rédigez un texte utile, précis et convaincant.";

    $('#pp-title').text(title || "Votre titre d'annonce");
    $('#pp-desc').text(desc);
    $('#pp-url').text(displayUrl());

    // Meta : adresse + téléphone (adresse peut venir de #adresse ou #adresse_campagne)
    const phone = $('input[name="numero"]').val ? $('input[name="numero"]').val().trim() : '';
    const address = ($('#adresse').val?.() ? $('#adresse').val().trim() : '') || ($('#adresse_campagne').val?.() ? $('#adresse_campagne').val().trim() : '');
    if (address || phone) {
      let meta = '';
    ...
    // Image fiable avec onload/onerror
    const imgSrc = firstPreviewImage();
    if (imgSrc) {
      const $img = $('#pp-image');
      $img.off('load.pm error.pm');
      $img.on('load.pm', function() {
        $(this).show();
        $('#pp-wrap').addClass('ads-split');
      }).on('error.pm', function() {
        $(this).hide();
  ...
  $(document).on('input',
    'input[name="titres[]"], input[name="descriptions[]"], input[name="titre_annexe[]"], input[name="desc1_annexe[]"], input[name="desc2_annexe[]"], input[name="url_annexe[]"], input[name="accroche_annexe[]"], input[name="site_annexe[]"], input[name="numero"], #adresse, input[name="Description_brève"], #url_campagne, #email_clients',
    function(){
      // sync adresse -> adresse_campagne (compat)
      const adr = $('#adresse').val()?.trim() || '';
      $('#adresse_campagne').val(adr);
      bindCounters();
      updatePhonePreview();
    }
  );
  ...
  $('#saveImagesBtn').click(function() {
    const images = imagePreviewContainer.find('img').map((_,img)=>img.src).get();
    const data = { idcampagne, idclient, images };
    data[csrfName] = csrfHash;
