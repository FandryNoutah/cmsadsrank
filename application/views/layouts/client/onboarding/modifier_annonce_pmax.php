...
<head>
  ...
  <title>Modifier l'annonce – PMax</title>
    ...
    <div class="col-12 col-lg-6 order-lg-2">
      <form action="<?= base_url('Client/update_annonce') ?>" method="POST" enctype="multipart/form-data">
        <div class="container-fluid pt-2">
          ...
          <div class="form-group">
            <label>Mot clé</label>
            <textarea rows="6" class="form-control" name="mot_cle"><?= $groupe[0]['mot_cle'] ?></textarea>
          </div>
        ...
        <div class="g-body" id="body-brief">
          <div class="g-field">
            ...
            <input type="text"
                  ...
                  data-max="60"
                  maxlength="60"
                  value="<?= htmlspecialchars($groupe[0]['description_breve'] ?? '') ?>">
            <span class="g-counter">0/60</span>
          </div>
        </div>
      </div>

          <!-- Assets visuels (dans un tiroir) -->
          <div class="g-block" id="sec-images">
            <div class="g-head" data-toggle="#body-images">
              <div>
                ...
                <div class="g-sub">Ajoutez/éditez les images utilisées par la campagne</div>
              </div>
            </div>
...
$(function(){
  ...
  function bindCounters(){
    $('.g-input').each(function(){
      ...
      const update=()=>{ const len=$i.val().length; if($c.length){$c.text(`${len}/${max}`).toggleClass('bad', max && len>max);} $i.toggleClass('is-invalid', max && len>max); };
  ...
  // PMax: affichage = host uniquement
  ...
  function updatePhonePreview() {
    ...
    const brief = $('input[name="description_breve"]').val()?.trim() || '';
    ...
    if (imgSrc) {
      ...
      $img.on('load.pm', function(){ $(this).show(); $('#pp-wrap').addClass('ads-split'); })
          .on('error.pm', function(){ $(this).hide(); $('#pp-wrap').removeClass('ads-split'); });
      $img.attr('src', imgSrc);
    } else {
      $('#pp-image').hide();
...
function annexeTpl(){
  ...
  /* ===== Live update ===== */
  $(document).on('input',
    'input[name="titres[]"], input[name="titres_longs[]"], input[name="descriptions[]"], input[name="description_breve"], input[name="titre_annexe[]"], input[name="desc1_annexe[]"], input[name="desc2_annexe[]"], input[name="url_annexe[]"], input[name="accroche_annexe[]"], input[name="site_annexe[]"], input[name="numero"], input[name="adresse"], #url_campagne',
