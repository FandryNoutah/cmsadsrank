					...
					<tbody>
						<?php foreach ($tache as $t):  ?>
							<?php if ($t->status == "planifié"): ?>
								<tr class="task-filter" data-type="<?= $t->type_tache ?>" data-am="<?= $t->AM; ?>" data-assigned="<?= $t->assigned_to ?>" style="<?= ($t->expired) ? "background-color: rgba(255, 0, 0, 0.05);" : "text-muted" ?>">
								<td>
										<h6 class="mb-0 ml-3">
											<?= $t->nom_client; ?>
										</h6>
				...
				<div class="col mb-3">
					<div class="card" style="border-radius: 8px;">
						<div class="card-body">
							...
							<?php foreach ($tache as $t): ?>
								<?php if ($t->status == "planifié"): ?>
									<div class="card mt-3 task-filter" data-type="<?= $t->type_tache ?>" data-am="<?= $t->AM; ?>" data-assigned="<?= $t->assigned_to ?>" style="<?= ($t->expired) ? "background-color: rgba(255, 0, 0, 0.05);" : "text-muted" ?>">
										<div class="card-body">
											<div class="d-flex">
												<?php if ($t->type_tache == 1): ?>
													<div class="mr-2">
