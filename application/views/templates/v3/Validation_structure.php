...
<div class="container">
  ...
  <div class="section">
    ...
    <table style="width:100%;border-collapse:collapse;border:1px solid #dee2e6;background:#fff">
      <thead style="background:#4EA5FE;color:#fff">
        <tr><th colspan="2" style="padding:12px;border:1px solid #dee2e6;text-align:center">Liste</th></tr>
      </thead>
      <tbody>
        <?php $hasContent=false; $i=0; foreach ($exlusions as $D): if ($D['exclusion'] != NULL): $hasContent=true;
          $exclusion = htmlspecialchars($D['exclusion']); $lines = explode("\n", $exclusion);
          $lineCount = count($lines);
          if ($lineCount > 21) { $firstPart = implode("\n", array_slice($lines,0,21)); $secondPart = implode("\n", array_slice($lines,21)); }
          else { $firstPart = $exclusion; $secondPart = ''; }
        ?>
          <tr>
